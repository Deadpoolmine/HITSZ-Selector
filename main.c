...
int main(int argc, char const *argv[])
{
    ...
    mergerOptions.bIsOptimise = FALSE;
    ...
    uiOpCnt = tpmm(mergerOptions, TABLE_R_NBLK + 1, TABLE_R_NBLK + TABLE_S_NBLK, 1, TABLE_R_NBLK, &uiNum, pBuf);
    dCheckBLKs(SM_POS, SM_POS + uiNum - 1, pBuf);
    printf(FONT_COLOR_GREEN "\nS和R的交集共 %ld 个元组\n" FONT_COLOR_END, uiOpCnt);
    ...
    uiNum = 0;
    uiOpCnt = tpmm(mergerOptions, TABLE_R_NBLK + 1, TABLE_R_NBLK + TABLE_S_NBLK, 1, TABLE_R_NBLK, &uiNum, pBuf);
    dCheckBLKs(7000, 7000 + uiNum, pBuf);
    printf("共%d\n", uiOpCnt);
#endif // !TEST
